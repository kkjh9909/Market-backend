...
class NeighborCommentLikeControllerTest {
    ...
    void addLike() throws Exception {
        ...
        mockMvc.perform(
                        post("/api/neighbor/comment/like/{commentId}", 5L)
                                .contentType(MediaType.APPLICATION_JSON)
                                .header("Authorization", "Bearer " + token)
                )
                .andExpect(status().isOk())
                .andDo(
                        MockMvcRestDocumentation.document("neighbor-comment-like-add",
                                ...
                                responseFields(
                                        ...
                                        subsectionWithPath("result.like_count").description("좋아요 개수")
                                )
                        )
                );
    }
    ...
    void deleteLike() throws Exception {
        ...
        mockMvc.perform(
                        delete("/api/neighbor/comment/like/{commentId}", 5L)
                                .contentType(MediaType.APPLICATION_JSON)
                                .header("Authorization", "Bearer " + token)
                )
                .andExpect(status().isOk())
                .andDo(
                        MockMvcRestDocumentation.document("neighbor-comment-like-delete",
                                preprocessRequest(prettyPrint()),
                                preprocessResponse(prettyPrint()),
                                requestHeaders(
                                        headerWithName("Authorization").description("인증 토큰")
